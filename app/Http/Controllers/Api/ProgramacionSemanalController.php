        ...
        try {
            ...
            $programacion = Planificacion_Asignatura::with([
                ...
                ->unique(function ($item) {
                    ...
                    return $item->id_modulo . '-' . $item->id_asignatura . '-' . ($item->horario->profesor->run_profesor ?? 'null');
                })
                ...
                ->map(function ($items, $dia) {
                    ...
                    $rows = $items->map(function ($planificacion) {
                        ...
                        return [
                            ...
                            'profesor' => [
                                'run' => $planificacion->horario->profesor->run_profesor ?? null,
                                'nombre' => $planificacion->horario->profesor->name ?? 'Sin asignar',
                                'email' => $planificacion->horario->profesor->email ?? null,
                            ]
                        ];
    ...
    public function obtenerReservaActiva($id_espacio)
    {
        try {
            ...
            if ($reserva->asignatura && $reserva->asignatura->id_asignatura) {
                ...
                $inscritos = Planificacion_Asignatura::where('id_asignatura', $reserva->asignatura->id_asignatura)
                    ->first();
                if ($inscritos) {
                    $inscritosData = [
