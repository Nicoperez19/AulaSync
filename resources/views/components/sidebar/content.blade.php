<nav aria-label="main" class="flex flex-col gap-4 my-[2rem] pt-2" style="color:white;">
    ...
    @can('monitoreo de espacios')
        <x-sidebar.link title="Monitoreo de Espacios"
            href="{{ $primerMapa ? route('plano.show', $primerMapa->id_mapa) : '#' }}" :isActive="request()->routeIs('plano.show')"
    ...
    @can('horarios por espacios')
        <x-sidebar.link title="Horarios por Espacios"
            href="{{ $sede ? route('espacios.show', $sede->id_sede) : route('dashboard') }}" :isActive="request()->routeIs('espacios.show')">
            <x-slot name="icon">
    ...
    @can('tablero academico')
        <x-sidebar.link title="Tablero Académico" href="{{ route('modulos.actuales') }}" :isActive="request()->routeIs('modulos.actuales')">
            <x-slot name="icon">
    ...
    @can('reportes')
        <x-sidebar.dropdown title="Reportes" :active="Str::startsWith(request()->route()->uri(), 'reporteria')">
            ...
            <x-sidebar.sublink title="Análisis por espacios" href="{{ route('reporteria.espacios') }}" :isActive="request()->routeIs('reporteria.espacios')" />
    ...
    @canany(['mantenedor de roles', 'mantenedor de permisos', 'mantenedor de universidades', 'mantenedor de facultades', 'mantenedor de areas academicas', 'mantenedor de carreras', 'mantenedor de pisos', 'mantenedor de espacios', 'mantenedor de reservas', 'mantenedor de asignaturas', 'mantenedor de mapas'])
