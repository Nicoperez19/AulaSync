<div class="relative" x-data="{ open: @entangle('mostrarDropdown') }">
    ...
    <button 
        @click="open = !open"
        type="button" 
        class="relative p-2 text-white bg-white/20 border border-white/40 rounded-md hover:bg-white/30 dark:hover:bg-dark-eval-2 focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-cloud-blue-500 transition-colors duration-200"
        ...
        @if($contadorNoLeidas > 0)
            <span class="absolute top-0 right-0 inline-flex items-center justify-center min-w-[20px] h-5 px-1.5 text-xs font-bold leading-none text-[#D2091E] bg-white border-2 border-[#D2091E] transform translate-x-1/2 -translate-y-1/2 rounded-full shadow-sm">
                ...
                <a 
                    ...
                    <div class="flex items-start">
                        ...
                        @if(!$notificacion->leida)
                            <div class="ml-2">
                                <span class="inline-block w-2 h-2 bg-[#D2091E] rounded-full"></span>
                            </div>
                        @endif
                    </div>
                </a>
