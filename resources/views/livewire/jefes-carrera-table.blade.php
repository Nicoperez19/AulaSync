...
<div>
    <div class="flex flex-col gap-4 mb-4 md:flex-row md:items-center md:justify-between">
        ...
        <div class="flex items-center text-sm text-gray-500 dark:text-gray-400">
            <span>Total: <strong>{{ $jefesCarrera->total() }}</strong> jefes de carrera</span>
        </div>
    </div>
        ...
        <table class="w-full text-sm text-center border-collapse table-auto min-w-max">
            ...
            <tbody>
                @forelse ($jefesCarrera as $index => $jefeCarrera)
                    <tr wire:key="jefe-row-{{ $jefeCarrera->id }}" class="{{ $index % 2 === 0 ? 'bg-white' : 'bg-gray-50' }}">
