<x-app-layout>
    ...
    <a href="http://127.0.0.1:8000/dashboard" class="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-gray-600 rounded-lg hover:bg-gray-700 transition-colors duration-200 shadow-md mb-4">
    ...
    <div class="p-6 bg-white rounded-lg shadow-md dark:bg-gray-800">
        <div class="p-3 rounded-lg bg-gray-50 mb-6">
            ...
            <div class="grid grid-cols-1 gap-2 text-sm md:grid-cols-2">
                <div>
                    ...
                    <span class="ml-2 text-gray-800">{{ $mapa->nombre_mapa ?? 'N/A' }}</span>
                </div>
                @if($mapa->piso)
                <div>
                ...
                <div>
                    ...
                    <span class="ml-2 text-gray-800">{{ $mapa->piso->facultad->nombre_facultad ?? 'N/A' }}</span>
                </div>
                @endif
                <div>
                    <span class="font-medium text-gray-600">Fecha de creación:</span>
                    <span class="ml-2 text-gray-800">{{ $mapa->created_at ? $mapa->created_at->format('d/m/Y H:i') : 'N/A' }}</span>
                </div>
            </div>
        </div>
        <script>
            window.mapaPisoId = "{{ $mapa->piso_id ?? '' }}";
        </script>
        ...
        <div class="flex flex-col gap-6 md:flex-row">
            ...
            <div class="w-full md:w-3/4">
                ...
                <div class="mb-4">
                    ...
                    @if($mapa->ruta_mapa)
                        <div class="mt-2">
                            ...
                            <img id="currentMapImage" src="{{ asset('storage/' . $mapa->ruta_mapa) }}" alt="Mapa actual" class="max-w-xs max-h-40 border rounded shadow">
                        </div>
                    @endif
                </div>
                ...
                <div class="flex justify-end gap-4 mt-4">
                    ...
                    <button id="saveMapBtn" type="button" class="hidden px-4 py-2 text-white bg-green-600 rounded hover:bg-green-700">Guardar Edición</button>
                </div>
            </div>
        </div>

        <!-- Formulario oculto para guardar -->
        <form id="saveMapForm" method="POST" action="{{ route('mapas.update', $mapa->id_mapa) }}" enctype="multipart/form-data" class="hidden">
            @csrf
            @method('PUT')
            <input type="text" name="nombre_mapa" id="nombre_mapa_form" value="{{ $mapa->nombre_mapa ?? '' }}">
            <input type="hidden" name="piso_id" id="piso_id" value="{{ $mapa->piso_id ?? '' }}">
    ...
    <script>
         document.addEventListener("DOMContentLoaded", function() {
            const selectPisoBtn = document.getElementById('selectPisoBtn');
            const pisoSelect = document.getElementById('piso');
            selectPisoBtn.style.display = 'none';
            pisoSelect.addEventListener('change', function() {
                selectPisoBtn.style.display = pisoSelect.value ? '' : 'none';
            });
            // Mostrar el botón al hacer clic en cualquier opción del select
            pisoSelect.addEventListener('click', function() {
                if (pisoSelect.value) {
                    selectPisoBtn.style.display = '';
                }
            });
            selectPisoBtn.addEventListener('click', function() {
                pisoSelect.dispatchEvent(new Event('change'));
            });
        });
        document.addEventListener("DOMContentLoaded", function() {
            // Variables globales
                ...
                fetch(`/espacios-por-piso/${pisoId}`)
                    ...
                    .then(data => {
                        ...
                        espacios.forEach(espacio => {
                            ...
                            // Selección del espacio (clic en el div)
                            espacioItem.addEventListener('click', function(e) {
 ...
 document.addEventListener("DOMContentLoaded", function() {
            ...
            // Cargar pisos al iniciar (corregido)
            const facultadId = elements.facultadId.value;
            if (facultadId) {
                fetch(`/pisos/${facultadId}`)
                    .then(response => {
                        if (!response.ok) {
                            console.error('Error HTTP al cargar pisos:', response.status);
                            return [];
                        }
                    ...
                    .then(data => {
                        ...
                        if (!data || data.length === 0) {
                            console.warn('No se recibieron pisos para la facultad:', facultadId);
                        }
                        data.forEach(piso => {
                            ...
                            if (facultadId === 'IT_LA') {
                                if (piso.numero_piso == 1) nombrePiso = 'CAUPOLICÁN 276';
                                else if (piso.numero_piso == 2) nombrePiso = 'VILLAGRÁN 220';
                                else if (piso.numero_piso == 3) nombrePiso = 'VILLAGRÁN 251';
                            }
            ...
            // Si tienes el id del piso actual, guárdalo en window.mapaPisoId antes de este script
        });
    </script>
