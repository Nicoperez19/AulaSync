...
<div class="space-y-6" x-data="{ 
    ...
    <div x-show="modalAbierto" x-cloak class="fixed inset-0 z-[9999] flex items-center justify-center p-4"
        style="background-color: rgba(0,0,0,0.5);" @click.self="cerrarModal()" @keydown.escape.window="cerrarModal()">
        ...
        <div x-show="modalAbierto" x-transition:enter="ease-out duration-300"
            ...
            class="bg-white rounded-xl shadow-2xl w-full max-w-lg flex flex-col" style="max-height: calc(100vh - 2rem);"
            @click.stop>
                ...
                <div class="flex flex-col max-h-full">
                    ...
                    <div class="px-6 py-4 bg-gray-50 border-b border-gray-200 flex-shrink-0">
                        <div class="grid gap-4 text-center"
                            :class="diaSeleccionado.por_realizar > 0 ? 'grid-cols-4' : 'grid-cols-3'">
                            <div class="bg-white rounded-lg p-3 shadow-sm"
                                title="Clases programadas que sí se realizaron">
                            ...
                            <div class="bg-white rounded-lg p-3 shadow-sm"
                                title="Clases que estaban programadas, pero no se realizaron">
                                <p class="text-2xl font-bold text-red-600" x-text="diaSeleccionado.no_realizadas"></p>
                                <p class="text-xs text-gray-500">No Registradas <i class="fas fa-info-circle ml-1"></i>
                                </p>
                            </div>
                            <div class="bg-white rounded-lg p-3 shadow-sm" x-show="diaSeleccionado.por_realizar > 0">
                                <p class="text-2xl font-bold text-yellow-600"
                                    x-text="diaSeleccionado.por_realizar || 0"></p>
                    ...
                    <div class="px-6 py-4 overflow-y-auto flex-1 min-h-0" style="max-height: 50vh;">
                        <template
                            x-if="diaSeleccionado.clases_no_realizadas_detalle && diaSeleccionado.clases_no_realizadas_detalle.length > 0">
                            <div>
                                <h4
                                    class="font-semibold text-gray-800 mb-3 flex items-center sticky top-0 bg-white py-2 z-10 -mx-6 px-6 border-b border-gray-100 shadow-sm">
                                ...
                                <div class="space-y-3">
                                    <template x-for="(clase, index) in diaSeleccionado.clases_no_realizadas_detalle"
                                        :key="index">
                                        <div
                                            class="p-4 bg-white rounded-lg border-l-4 border-red-400 shadow-sm hover:shadow-md transition-shadow">
                                            <div class="flex justify-between items-start gap-3">
                                                <div class="flex-1 min-w-0">
                                                    <p class="font-semibold text-gray-800 truncate"
                                                        x-text="clase.asignatura || 'Asignatura no disponible'"></p>
                                                    <p class="text-sm text-gray-600 mt-1 flex items-center">
                                                        ...
                                                        <span
                                                            x-text="clase.profesor || 'Profesor no disponible'"></span>
                                                    </p>
                                                    <div class="flex items-center gap-2 mt-2 flex-wrap">
                                                        <span
                                                            ...
                                                            <span
                                                                x-text="(clase.modulos || 1) + ' módulo' + ((clase.modulos || 1) > 1 ? 's' : '')"></span>
                                                            <template x-if="clase.hora">
                                                                <span class="ml-1 text-gray-500"
                                                                    x-text="'(' + clase.hora + ')'"></span>
                                                            </template>
                                                        </span>
    ...
    <div class="p-4 bg-blue-50 rounded-lg border border-blue-200">
        <div class="flex items-center gap-2 text-blue-700">
            ...
            <span class="font-medium">Período académico: {{ $periodo ?? 'No definido' }} | Mes:
                {{ \Carbon\Carbon::create($anio, $mes, 1)->translatedFormat('F Y') }}</span>
        </div>
    </div>
    ...
    @if($totalRealizadas == 0 && $totalNoRealizadas == 0)
        ...
        <div class="p-8 bg-gray-50 rounded-lg border border-gray-200 text-center">
            <div class="flex flex-col items-center gap-4">
                ...
                <p class="text-gray-500 max-w-md">No se encontraron planificaciones de asignaturas para el período
                    <strong>{{ $periodo ?? 'actual' }}</strong>. Esto puede deberse a que no hay horarios cargados o el
                    período aún no tiene datos.
                </p>
            </div>
        </div>
    @else
        ...
        <div class="grid grid-cols-1 gap-4 md:grid-cols-4">
            <div class="p-4 bg-white rounded-lg shadow border border-gray-200">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm text-gray-600" title="Clases programadas que sí se realizaron">Clases Registradas
        ...
        <div class="p-6 bg-white rounded-lg shadow border border-gray-200">
            ...
            <!-- Vista: Semana Actual -->
            <div x-show="vistaDetalle === 'semana'" x-transition:enter="transition ease-out duration-200"
                ...
                <div class="mb-4 p-3 bg-purple-50 rounded-lg border border-purple-200">
                    <p class="text-sm text-purple-700">
                        ...
                        Semana del <strong>{{ $inicioSemana->format('d/m/Y') }}</strong> al
                        <strong>{{ $finSemana->format('d/m/Y') }}</strong>
                    ...
                    @foreach($diasSemana as $dia => $datos)
                        ...
                        <div @if($esClickeable) @click="abrirModal('{{ $dia }}')" @endif
                            ...
                            <div class="text-center mb-3">
                                <p
                                    class="text-xs uppercase font-semibold {{ $datos['es_hoy'] ? 'text-purple-600' : ($esSabado ? 'text-amber-600' : ($esFuturo ? 'text-gray-400' : 'text-gray-500')) }}">
                                    {{ ucfirst($datos['dia_nombre']) }}
                                </p>
                                <p
                                    class="text-2xl font-bold {{ $datos['es_hoy'] ? 'text-purple-700' : ($esFuturo ? 'text-gray-400' : 'text-gray-800') }}">
                                    {{ $datos['fecha']->format('d') }}
                                </p>
                                @if($datos['es_hoy'])
                                    <span
                                        class="inline-block mt-1 px-2 py-0.5 bg-purple-600 text-white text-xs rounded-full">Hoy</span>
                                @endif
                                @if($esSabado && !$esFuturo)
                                    <span class="inline-block mt-1 px-2 py-0.5 bg-amber-500 text-white text-xs rounded-full">Hasta
                                        13:00</span>
                                @endif
                                @if($esFuturo && !$esDiaSinClases)
                                    <span
                                        class="inline-block mt-1 px-2 py-0.5 bg-gray-300 text-gray-600 text-xs rounded-full">Pendiente</span>
                                @endif
                            </div>
                            ...
                            @else
                                <div class="space-y-2">
                                    <div class="flex justify-between items-center">
                                        ...
                                        <span
                                            class="px-2 py-0.5 bg-green-100 text-green-700 rounded text-xs font-bold">{{ $datos['realizadas'] }}</span>
                                    </div>
                                    <div class="flex justify-between items-center">
                                        <span class="text-xs text-gray-500">No registradas</span>
                                        <span
                                            class="px-2 py-0.5 bg-red-100 text-red-700 rounded text-xs font-bold">{{ $datos['no_realizadas'] }}</span>
                                    </div>
                                    @if(isset($datos['por_realizar']) && $datos['por_realizar'] > 0)
                                        <div class="flex justify-between items-center">
                                            <span class="text-xs text-gray-500">Por realizar</span>
                                            <span
                                                class="px-2 py-0.5 bg-yellow-100 text-yellow-700 rounded text-xs font-bold">{{ $datos['por_realizar'] }}</span>
                                        </div>
                                    @endif
                                    <div class="flex justify-between items-center">
                                        <span class="text-xs text-gray-500">Recuperadas</span>
                                        <span
                                            class="px-2 py-0.5 bg-blue-100 text-blue-700 rounded text-xs font-bold">{{ $datos['recuperadas'] ?? 0 }}</span>
                                    </div>
                                        ...
                                        <div class="pt-2 border-t border-gray-100">
                                            <div class="flex items-center gap-3 h-8">
                                                <div class="flex-1 h-3 bg-gray-200 rounded-lg overflow-hidden shadow-sm">
                                                    <div class="h-full {{ $porcentaje >= 80 ? 'bg-green-500' : ($porcentaje >= 50 ? 'bg-yellow-500' : 'bg-red-500') }} transition-all duration-300"
                ...
                @php
                    ...
                    $totalSemanaRecuperadas = collect($diasSemana)->sum(function ($d) {
                        return $d['recuperadas'] ?? 0;
                    });
                ...
                <div class="mt-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                        ...
                        <div class="text-center">
                            <p
                                class="text-2xl font-bold {{ $porcentajeSemana >= 80 ? 'text-green-600' : ($porcentajeSemana >= 50 ? 'text-yellow-600' : 'text-red-600') }}">
                                {{ $porcentajeSemana }}%
                            </p>
            ...
            <div x-show="vistaDetalle === 'calendario'" x-cloak x-transition:enter="transition ease-out duration-200"
                x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100">
                @php
                ...
                <div class="border border-gray-200 rounded-lg overflow-hidden">
                    ...
                    <div style="display: grid; grid-template-columns: repeat(6, 1fr);"
                        ...
                        @foreach(['Lun', 'Mar', 'Mié', 'Jue', 'Vie', 'Sáb'] as $diaSemana)
                            <div
                                class="p-2 text-center text-xs font-semibold text-gray-600 {{ $diaSemana === 'Sáb' ? 'bg-amber-100' : '' }}">
                                {{ $diaSemana }}
                                @if($diaSemana === 'Sáb')
                            ...
                            @else
                                ...
                                <div @if($esClickeableCal) @click="abrirModal('{{ $item['fecha_key'] }}')" @endif
                                    ...
                                    <div class="flex justify-between items-start mb-1">
                                        <span
                                            class="text-sm font-semibold {{ $item['es_hoy'] ? 'text-purple-700' : ($esFuturoCal ? 'text-gray-400' : 'text-gray-700') }}">
                                            {{ $item['dia'] }}
                                        </span>
                                        @if($item['es_hoy'])
                                    ...
                                    @elseif($tieneClases)
                                        ...
                                        <div class="mt-2 h-6">
                                            <div class="h-2.5 bg-gray-200 rounded-md overflow-hidden shadow-sm">
                                                <div class="h-full {{ $porcentajeCal >= 80 ? 'bg-green-500' : ($porcentajeCal >= 50 ? 'bg-yellow-500' : 'bg-red-500') }} transition-all duration-300"
                                                    style="width: {{ $porcentajeCal }}%"></div>
                                            </div>
                                        </div>
                                    @else
                                ...
                                @forelse($diasDelMes as $dia => $datos)
                                    ...
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="px-4 py-2">
                                            <div class="flex items-center gap-3 h-8">
                                                <div class="flex-1 h-3 bg-gray-200 rounded-lg overflow-hidden shadow-sm">
                                                    <div class="h-full {{ $colorBarraTabla }} transition-all duration-300"
                                                        style="width: {{ $porcentajeTabla }}%"></div>
                                                </div>
                                                <span
                                                    class="text-xs font-bold text-gray-700 w-10 text-right">{{ $porcentajeTabla }}%</span>
                                            </div>
                                        </td>
                                    </tr>
                                @empty
            ...
            <div x-show="vistaDetalle === 'filtrado'" x-cloak x-transition:enter="transition ease-out duration-200"
                ...
                <div class="mb-4 p-4 bg-purple-50 rounded-lg border border-purple-200">
                    <div class="flex flex-col md:flex-row md:items-end gap-4">
                        ...
                        <div>
                            <button @click="filtrar()" :disabled="cargando"
                ...
                <template x-if="error">
                    <div class="mb-4 p-4 bg-red-50 rounded-lg border border-red-200">
                        <p class="text-red-700"><i class="fas fa-exclamation-circle mr-2"></i><span x-text="error"></span>
                        </p>
                    </div>
                </template>
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="p-3 bg-gray-100 rounded-lg border border-gray-200">
                            <p class="text-sm text-gray-700">
                                ...
                                Mostrando estadísticas del <strong
                                    x-text="new Date(fechaInicio + 'T00:00:00').toLocaleDateString('es-CL')"></strong>
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                            <div class="p-4 bg-white rounded-lg shadow border border-gray-200 text-center">
                                ...
                                <p class="text-xs text-green-500"
                                    x-text="datosFiltrados.porcentaje_realizadas + '% del total'"></p>
                            </div>
                            <div class="p-4 bg-white rounded-lg shadow border border-gray-200 text-center">
                                ...
                                <p class="text-xs text-red-500"
                                    x-text="datosFiltrados.porcentaje_no_realizadas + '% del total'"></p>
                            </div>
                            <div class="p-4 bg-white rounded-lg shadow border border-gray-200 text-center">
                                ...
                                <p class="text-xs text-blue-500"
                                    x-text="datosFiltrados.porcentaje_recuperadas + '% de no realizadas'"></p>
                            </div>
                            <div class="p-4 bg-white rounded-lg shadow border border-gray-200 text-center">
                                ...
                                <p class="text-xs text-purple-500"
                                    x-text="'Promedio: ' + datosFiltrados.promedio_diario + '/día'"></p>
                            </div>
                        </div>
                                    ...
                                    <div>
                                        <div class="flex justify-between items-center mb-1">
                                            ...
                                            <span class="text-sm font-bold text-blue-600"
                                                x-text="datosFiltrados.porcentaje_recuperadas + '%'"></span>
                                        </div>
                                        <div class="h-3 bg-gray-200 rounded-full overflow-hidden">
                                    ...
                                    <div class="pt-4 border-t border-gray-200 space-y-3">
                                        <div class="flex justify-between items-center">
                                            <span class="text-sm text-gray-600"><i
                                                    class="fas fa-calendar-day mr-2 text-gray-400"></i>Días en el
                                                rango</span>
                                            <span class="font-semibold text-gray-800"
                                                x-text="datosFiltrados.dias_totales"></span>
                                        </div>
                                        <div class="flex justify-between items-center">
                                            <span class="text-sm text-gray-600"><i
                                                    class="fas fa-briefcase mr-2 text-gray-400"></i>Días laborales</span>
                                            <span class="font-semibold text-gray-800"
                                                x-text="datosFiltrados.dias_laborales"></span>
                                        </div>
                                        <div class="flex justify-between items-center">
                                            <span class="text-sm text-gray-600"><i
                                                    class="fas fa-clock mr-2 text-gray-400"></i>Promedio diario</span>
                                            <span class="font-semibold text-gray-800"
                                                x-text="datosFiltrados.promedio_diario + ' clases'"></span>
                                        </div>
                                        <div class="flex justify-between items-center">
                                            <span class="text-sm text-gray-600"><i
                                                    class="fas fa-exclamation-triangle mr-2 text-orange-400"></i>Pendientes
                                                recuperar</span>
                                            <span class="font-semibold text-orange-600"
                                                x-text="datosFiltrados.pendientes"></span>
                                        </div>
                                    </div>
                                </div>
                            </div>
        ...
        <div class="p-6 bg-blue-50 rounded-lg border border-blue-200">
            ...
            <ul class="text-sm text-blue-700 space-y-2">
                ...
                <li><strong>Período:</strong> Datos del mes de
                    {{ strval(\Carbon\Carbon::create($anio, $mes, 1)->locale('es')->isoFormat('MMMM')) }} de {{ $anio }}
                </li>
            </ul>
        </div>
    @endif
